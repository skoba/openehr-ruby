...
describe EventContext do
  ...
  it 'should be an instance of EventContext' do
    expect(@event_context).to be_an_instance_of EventContext
  end

  it 'start_time should be assigned properly' do
    expect(@event_context.start_time.value).to eq('2009-11-13T20:46:57')
  end
  ...
  it 'setting should be assigned properly' do
    expect(@event_context.setting.defining_code.code_string).to eq('225')
  end
  ...
  it 'participations should be properly assigned' do
    expect(@event_context.participations.size).to be_equal 5
  end
  ...
  it 'location should be assigned properly' do
    expect(@event_context.location).to eq('ehime')
  end
