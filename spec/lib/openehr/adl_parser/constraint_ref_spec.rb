  ...
  context 'Constraint Refs' do
    ...
    context 'constraint ref node' do
      ...
      it 'is an instance of ConstrantRef' do
        expect(@cr).to be_an_instance_of OpenEHR::AM::Archetype::ConstraintModel::ConstraintRef
      end

      it 'path is /content[at0001]/items[at0002]/value/defining_code' do
        expect(@cr.path).to eq('/content[at0001]/items[at0002]/value/defining_code')
      end

      it 'reference is ac0001' do
        expect(@cr.reference).to eq('ac0001')
      end
    end
  end
end
