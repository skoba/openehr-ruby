...
describe ADLParser do
  context 'Constraint Bindings' do
    ...
    it 'contraint bindings size is two' do
      expect(@ao.constraint_bindings.size).to be 2
    end
    ...
    it 'SNOMED_CT binds local ac0001 to http://terminology.org/?terminology_id=snomed_ct&&has_relation=102002;with_target=128004' do
      expect(@ao.constraint_binding(:terminology => 'SNOMED_CT', :code => 'ac0001').
        value).to eq('http://terminology.org?terminology_id=snomed_ct&&has_relation=102002;with_target=128004')
    end

    it 'ICD10 binds local ac0001 to http://terminology.org/?terminology_id=icd10&&has_relation=a2;with_target=b19' do
      expect(@ao.constraint_binding(:terminology => 'ICD10', :code => 'ac0001').
        value).to eq('http://terminology.org?terminology_id=icd10&&has_relation=a2;with_target=b19')
    end
  end
end
